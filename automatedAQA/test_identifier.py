...
def check_results_exist(main_folder):
    ...
    if results_dir.is_dir():
        ans = easygui_qt.ynbox(msg=f'The "{results_dir.name}" folder already exists. Do you want to continue and overwrite it?',default_choice='No', cancel_choice='No')
        if ans:
            print(f'Okay. Clearing {results_dir.name}')
            shutil.rmtree(results_dir)
        else:
            easygui_qt.msgbox(msg = 'Okay. Clearing {results_dir.name}')
